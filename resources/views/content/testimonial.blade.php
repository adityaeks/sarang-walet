...
<div class="row">
    <div class="card">
        <div class="card-body">
            ...
            <a href="{{ route('testimonial.create') }}" class="btn btn-lg bg-gradient-success mb-3" data-bs-toggle="modal" data-bs-target="#addDataModal">Add Data</a>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <form action="{{ route('testimonial.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <label class="font-weight-bold">Coment</label>
                                    <input type="text" class="form-control @error('coment') is-invalid @enderror" name="coment" value="{{ old('coment') }}" placeholder="Masukkan Judul Post">
                                    <!-- error message untuk coment -->
                                    @error('coment')
                                        <div class="alert alert-danger mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                ...
                <table class="table" id="alldata">
                    <thead class="text-center text-dark">
                        <tr>
                            ...
                            <th class="fs-4" scope="col">Coment</th>
                    ...
                    <tbody class="text-center">
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <th scope="row">
                                    <img src="{{ asset('/storage/images/testimonial/'.$item->image) }}" class="rounded" style="width: 16%">
                                </th>
                                <th class="fs-5" scope="row">{{ $item->name }}</th>                                
                                <th class="fs-5" scope="row">{{ $item->coment }}</th>                                
                                <th>
                                    <div class="d-flex justify-content-center">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <form action="{{ route('testimonial.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                                                            ...
                                                            <div class="form-group">
                                                                <label class="font-weight-bold">Coment</label>
                                                                <input type="text" class="form-control @error('coment') is-invalid @enderror" name="coment" value="{{ old('coment', $item->coment) }}" placeholder="Masukkan Judul Post">
                                                                <!-- error message untuk coment -->
                                                                @error('coment')
                                                                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                                                                @enderror
                                        ...
                                        <form  onsubmit="return confirm('Are you sure want to delete it ?');" action="{{ route('testimonial.delete', $item->id) }}" method="post"">    
                                            @csrf
                                            @method('DELETE')
